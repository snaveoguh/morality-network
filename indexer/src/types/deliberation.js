...
export const CLAIM_SOURCE_KINDS = ["headline", "source_statement", "user_submitted", "derived"];
export const INTERPRETATION_KINDS = ["discussion", "claim", "counterclaim", "evidence", "source"];
export const OUTCOME_STATES = ["unresolved", "resolved_true", "resolved_false", "contested"];
...
function normalizeClaimSentence(text) {
    let value = text
        .replace(/\s+/g, " ")
        .replace(/^["'“”]+|["'“”]+$/g, "")
...
function deriveClaim(identifier, entityHash, createdAt) {
...
function interpretationFromFeedItem(item, entityHash) {
...
function collectEvidence(entityHash, interpretations) {
