...
import { agentConsumerCursor, agentEvent, agentMemory, aiInvocation, articleArchive, editorialArchive, entity, feedItem, marketplaceOrder, scannerLaunch, swarmState, traderState, } from "../../ponder.schema";
import { fetchCanonicalClaimFromSource } from "../claim-extract";
import { CLAIM_SOURCE_KINDS, DELIBERATION_SCHEMA_VERSION, INTERPRETATION_KINDS, OUTCOME_STATES, buildCanonicalDeliberationGraph, } from "../types/deliberation";
...
function hasWorkerWriteAccess(authHeader) {
...
function parseArchiveLimit(value, fallbackValue = MAX_ARCHIVE_LIMIT) {
...
function parseEntityHash(value) {
    if (!value)
        return undefined;
...
function parseActionTypes(value) {
    if (!value)
        return undefined;
    const parts = value.split(",").map((v) => Number(v.trim()));
    if (parts.length === 0 || parts.some((v) => !Number.isInteger(v)))
        return undefined;
    const unique = Array.from(new Set(parts));
    if (unique.some((v) => v < 0 || v > 4))
        return undefined;
    return unique;
}
...
function parseScannerDex(value) {
    if (!value)
        return undefined;
    if (value === "uniswap-v3" || value === "aerodrome")
        return value;
    return undefined;
}
...
function parseAddressLoose(value) {
...
function actionTypeName(actionType) {
    switch (actionType) {
        case 0:
            return "rate";
        case 1:
...
function serializeFeedRow(row, entityRow) {
    return {
        id: row.id,
...
function serializeScannerLaunch(row) {
    return {
        id: row.id,
...
function serializeArticleArchiveRow(row) {
    return {
        hash: row.id,
        id: row.feedItemId,
        title: row.title,
        ...
        bias: safeJsonParse(row.biasJson, null),
        tags: safeJsonParse(row.tagsJson, []),
        canonicalClaim: row.canonicalClaim ?? undefined,
        preservedLinks: safeJsonParse(row.preservedLinksJson, []),
...
function computeEditorialContentHash(editorial) {
    const primary = (editorial.primary ?? {});
...
function serializeEditorialArchiveRow(row) {
...
function serializeSwarmStateRow(row) {
    return {
        generatedAt: row.generatedAt,
        scannedItems: row.scannedItems ?? 0,
        clusters: safeJsonParse(row.clustersJson, []),
        contradictionFlags: safeJsonParse(row.contradictionFlagsJson, []),
        updatedAt: Number(row.updatedAt),
    };
}
function serializeTraderStateRow(row) {
    return {
        executionMode: row.executionMode,
        config: safeJsonParse(row.configJson, null),
        report: safeJsonParse(row.primaryReportJson, null),
        parallel: safeJsonParse(row.parallelReportsJson, []),
        readiness: safeJsonParse(row.primaryReadinessJson, null),
        parallelReadiness: safeJsonParse(row.parallelReadinessJson, []),
        performance: safeJsonParse(row.primaryPerformanceJson, null),
        parallelPerformance: safeJsonParse(row.parallelPerformanceJson, []),
        positions: safeJsonParse(row.primaryPositionsJson, []),
        parallelPositions: safeJsonParse(row.parallelPositionsJson, []),
        updatedAt: Number(row.updatedAt),
    };
}
function serializeAgentEventRow(row) {
    return {
        id: row.id,
        from: row.fromAgent,
        to: row.toAgent,
        ...
        meta: safeJsonParse(row.metaJson, undefined),
        source: row.source,
        timestamp: Number(row.timestampMs),
        persistedAt: Number(row.persistedAt),
    };
}
function serializeAgentConsumerCursorRow(row) {
    return {
        id: row.id,
...
function serializeAIInvocationRow(row) {
    return {
        id: row.id,
        task: row.task,
...
function normalizeAgentEventInput(value) {
...
function normalizeAIInvocationInput(value) {
    if (!value || typeof value !== "object")
        return null;
    const input = value;
    ...
    const totalTokens = Math.max(0, Math.floor(safeNumber(input.totalTokens, inputTokens + outputTokens)));
    const latencyMs = Math.max(0, Math.floor(safeNumber(input.latencyMs, 0)));
    const estimatedCostMicrousd = BigInt(Math.max(0, Math.floor(safeNumber(input.estimatedCostMicrousd, 0))));
    const error = parseOptionalText(input.error);
    return {
        id: parseOptionalText(input.id) ??
...
async function persistAgentEvents(c, messages, source) {
...
async function persistAIInvocation(c, invocation) {
    const existing = await c.db
        .select()
        .from(aiInvocation)
...
function normalizeArchivedArticleInput(value, nowIso) {
    if (!value || typeof value !== "object")
        return null;
    const input = value;
    ...
    const canonicalClaim = parseOptionalText(input.canonicalClaim) ??
...
async function upsertArticleArchiveRow(c, record) {
    const existing = await c.db
        .select()
        .from(articleArchive)
    ...
    const newCanonicalClaim = row.canonicalClaim && row.canonicalClaim !== "Claim unavailable."
    ...
    await c.db
        ...
        tagsJson: JSON.stringify(record.tags ?? safeJsonParse(row.tagsJson, [])),
        canonicalClaim,
        preservedLinksJson: JSON.stringify(record.preservedLinks ?? safeJsonParse(row.preservedLinksJson, [])),
        firstSeenAt: row.firstSeenAt,
        lastSeenAt: record.lastSeenAt,
...
async function upsertEditorialArchiveRow(c, hash, editorial, generatedByRaw) {
    ...
    const hasMarketImpact = Boolean(editorial.marketImpact) &&
        Array.isArray(editorial.marketImpact.affectedMarkets) &&
        editorial.marketImpact.affectedMarkets.length > 0;
    const payload = {
        ...editorial,
        entityHash: hash,
        generatedAt: nowIso,
...
ponder.get("/api/v1/entities/:entityHash/feed", async (c) => {
    ...
    if (cursor !== undefined)
        conditions.push(lt(feedItem.timestamp, cursor));
...
ponder.get("/api/v1/feed/global", async (c) => {
    ...
    if (cursor !== undefined)
        conditions.push(lt(feedItem.timestamp, cursor));
    if (from !== undefined)
        conditions.push(gte(feedItem.timestamp, from));
    if (to !== undefined)
        conditions.push(lte(feedItem.timestamp, to));
    if (actionTypes && actionTypes.length > 0) {
        conditions.push(inArray(feedItem.actionType, actionTypes));
    }
    if (actor !== undefined)
        conditions.push(eq(feedItem.actor, actor));
    const whereClause = conditions.length === 0 ? null : conditions.length === 1 ? conditions[0] : and(...conditions);
    ...
    const entityHashes = Array.from(new Set(baseItems.map((item) => item.entityId).filter((hash) => hash !== ZERO_ENTITY_HASH)));
    const entityRows = entityHashes.length > 0
...
ponder.get("/api/v1/agents/events", async (c) => {
    ...
    const conditions = [];
    if (cursor !== undefined)
        conditions.push(lt(agentEvent.timestampMs, cursor));
    if (since !== undefined)
        conditions.push(gte(agentEvent.timestampMs, since));
    if (fromAgent)
        conditions.push(eq(agentEvent.fromAgent, fromAgent));
    if (toAgent)
        conditions.push(eq(agentEvent.toAgent, toAgent));
...
ponder.get("/api/v1/agents/events/summary", async (c) => {
    ...
    if (unauthorized)
        return unauthorized;
    const windowMs = Math.max(60_000, Math.min(7 * 24 * 60 * 60 * 1000, Math.floor(safeNumber(c.req.query("windowMs"), 15 * 60 * 1000))));
...
ponder.post("/api/v1/agents/cursors/:consumer", async (c) => {
    ...
    const input = body;
    const lastTimestampMs = BigInt(Math.max(0, Math.floor(safeNumber(input.lastTimestampMs, 0))));
...
ponder.get("/api/v1/ai/usage", async (c) => {
    ...
    const conditions = [];
    if (since !== undefined)
        conditions.push(gte(aiInvocation.createdAt, since));
    if (cursor !== undefined)
        conditions.push(lt(aiInvocation.createdAt, cursor));
    if (task)
        conditions.push(eq(aiInvocation.task, task));
    if (provider)
        conditions.push(eq(aiInvocation.provider, provider));
    if (status)
        conditions.push(eq(aiInvocation.status, status));
    const whereClause = conditions.length === 0 ? null : conditions.length === 1 ? conditions[0] : and(...conditions);
    const rows = await (whereClause
        ? c.db
            .select()
...
ponder.get("/api/v1/ai/usage/summary", async (c) => {
    ...
    const mergeIntoBucket = (map, key, row) => {
    ...
    const summarizeBuckets = (map, keyName) => Array.from(map.entries())
        .map(([key, bucket]) => ({
        [keyName]: key,
        ...bucket,
...
ponder.get("/api/v1/scanner/launches", async (c) => {
    ...
    const conditions = [];
    if (cursor !== undefined)
        conditions.push(lt(scannerLaunch.discoveredAt, cursor));
    if (minScore > 0)
        conditions.push(gte(scannerLaunch.score, minScore));
    if (dex)
        conditions.push(eq(scannerLaunch.dex, dex));
    if (source)
        conditions.push(eq(scannerLaunch.source, source));
    const whereClause = conditions.length === 0 ? null : conditions.length === 1 ? conditions[0] : and(...conditions);
    const totalStored = (await (whereClause
        ? c.db.select().from(scannerLaunch).where(whereClause)
        : c.db.select().from(scannerLaunch))).length;
    const rows = await (whereClause
        ? c.db
            .select()
            .from(scannerLaunch)
...
const handleScannerSync = async (c) => {
    if (!hasWorkerWriteAccess(c.req.header("authorization"))) {
        return c.json({ error: "unauthorized" }, 401);
    }
    ...
    for (const pair of pairs) {
        if (upserted >= limit)
            break;
        if (String(pair.chainId ?? "").toLowerCase() !== "base")
            continue;
        ...
        if (!pairAddress || !baseAddress || !quoteAddress)
            continue;
...
ponder.post("/api/v1/swarm/latest", async (c) => {
    ...
    const previousContradictions = new Set();
    if (existing[0]) {
        const priorClusters = safeJsonParse(existing[0].clustersJson, []);
        const priorFlags = safeJsonParse(existing[0].contradictionFlagsJson, []);
        for (const cluster of priorClusters) {
            const clusterId = parseText(cluster.clusterId).trim();
            if (!clusterId)
                continue;
            previousClusters.set(clusterId, JSON.stringify({
    ...
    for (const cluster of clusters.slice(0, 20)) {
        ...
        emittedEvents.push({
            ...
            payload: {
                ...
                hasContradictions: Array.isArray(record.contradictionFlags) && record.contradictionFlags.length > 0,
...
ponder.post("/api/v1/trading/state", async (c) => {
    ...
    const values = {
        ...
        parallelReadinessJson: JSON.stringify(Array.isArray(input.parallelReadiness) ? input.parallelReadiness : []),
        primaryPerformanceJson: JSON.stringify(input.performance ?? null),
        parallelPerformanceJson: JSON.stringify(Array.isArray(input.parallelPerformance) ? input.parallelPerformance : []),
        primaryPositionsJson: JSON.stringify(Array.isArray(input.positions) ? input.positions : []),
        parallelPositionsJson: JSON.stringify(Array.isArray(input.parallelPositions) ? input.parallelPositions : []),
        updatedAt,
    };
    if (existing[0]) {
        await c.db
...
ponder.get("/api/v1/archive/editorials/market-impact", async (c) => {
    ...
    const records = rows
        ...
        .filter((record) => Boolean(record.entityHash) &&
...
ponder.post("/api/v1/archive/editorials/upsert", async (c) => {
    ...
    const hash = parseEntityHash(body.hash);
    ...
    const payload = await upsertEditorialArchiveRow(c, hash, editorial, generatedBy);
    return c.json({
        ok: true,
        editorial: payload,
        meta: { generatedAt: Date.now() },
...
ponder.post("/api/v1/archive/editorials/:hash/mark-onchain", async (c) => {
    ...
    const body = await c.req.json().catch(() => null);
    const txHash = body && typeof body === "object" ? parseOptionalText(body.txHash) : null;
    ...
    const nowIso = new Date().toISOString();
    const payload = serializeEditorialArchiveRow(row);
...
ponder.post("/api/v1/marketplace/orders", async (c) => {
    ...
    const { orderHash, tokenContract, tokenId, maker, priceWei, expiresAt, orderJson, signature, collection, } = body;
    // Validate required fields
    if (typeof orderHash !== "string" || !orderHash ||
