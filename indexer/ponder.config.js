...
function parsePositiveInt(input, fallbackValue) {
    if (!input)
...
const rpcTransport = rpcUrls.length > 1
...
const postgresConnectionString = process.env.DATABASE_PRIVATE_URL?.trim() ||
    process.env.DATABASE_URL?.trim();
const database = databaseMode === "postgres" && postgresConnectionString
    ? {
        kind: "postgres",
