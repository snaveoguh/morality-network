...
const SLIDES: readonly { title: string; art: string; body: string }[] = [
  ...
  GET /performance ──▶ track record`,
    body: "You can mirror the engine's trades by polling the public API. Fetch open positions from /api/trading/positions?openOnly=1 to see current entries with side, size, and entry price. Poll /api/trading/signals/live for the same composite signals the engine acts on, including per-component breakdowns. Use /api/trading/performance for win rate, PnL, and Sharpe ratio to evaluate before committing capital.",
  },
  {
