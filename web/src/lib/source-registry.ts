...
import { DEFAULT_FEEDS, type FeedItem, type FeedSource } from "./rss";
...
const COMMUNITY_SOURCES: CanonicalSourceRecord[] = [
  {
    id: "reddit",
    name: "Reddit",
    category: "Community",
    ...
    bias: getSourceBias("reddit.com") ?? null,
    active: true,
  },
  {
    id: "4chan",
    ...
    bias: getSourceBias("4chan.org") ?? null,
    active: true,
  },
];

let registryCache: CanonicalSourceRecord[] | null = null;
...
function deriveHomepageUrl(raw: string): string {
  const parsed = safeUrl(raw);
  if (!parsed) return raw;
  return `${parsed.protocol}//${parsed.host}`;
}

function deriveSourceKind(source: FeedSource): SourceKind {
...
function mergeFeedSource(
  map: Map<string, CanonicalSourceRecord>,
  source: FeedSource,
) {
  const id = slugify(source.name);
  const existing = map.get(id);
  const feedUrl = normalizeUrl(source.url);
  const fallbackUrl = source.fallbackUrl ? normalizeUrl(source.fallbackUrl) : "";
  ...
  const bias =
    getSourceBias(source.name) ||
    getSourceBias(feedUrl) ||
    (feedHost ? getSourceBias(feedHost) : null) ||
    null;
  ...
  map.set(id, {
    ...
    domains: [feedHost, fallbackHost].filter(Boolean) as string[],
    bias,
    active: true,
  });
}

export function getCanonicalSourceRegistry(): CanonicalSourceRecord[] {
  if (registryCache) return registryCache;

  const merged = new Map<string, CanonicalSourceRecord>();
  ...
  registryCache = Array.from(merged.values()).sort((a, b) =>
    a.name.localeCompare(b.name)
  );
  return registryCache;
}

export function resolveCanonicalSource(input: {
  ...
  for (const host of hosts) {
    const match = registry.find((source) =>
      source.domains.some((domain) => host === domain.toLowerCase())
    );
...
export function extractSourceNames(items: FeedItem[]): string[] {
