...
export const BIAS_COUNTRY_TO_ISO: Record<string, string[]> = {
  US: ["US"],
  UK: ["GB"],
  Canada: ["CA"],
  Qatar: ["QA"],
...
export function normalizeBiasCountry(biasCountry: string | undefined): string[] {
  if (!biasCountry) return [];
  return BIAS_COUNTRY_TO_ISO[biasCountry] ?? [];
}

// ── ISO → display label ────────────────────────────────────────────────────

export const ISO_TO_LABEL: Record<string, string> = {
  US: "US",
  GB: "UK",
  CA: "CA",
  QA: "QA",
...
// ── Story country detection patterns ───────────────────────────────────────
// Keyword-based — scans title + description, similar to KEYWORD_PATTERNS in rss.ts
...
export function detectStoryCountries(title: string, description: string): string[] {
  const text = `${title} ${description}`;
  const matched = new Set<string>();
  for (const { iso, pattern } of COUNTRY_PATTERNS) {
