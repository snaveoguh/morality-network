...
export type AgentEvidenceKind = "report" | "document" | "data" | "link";

export interface AgentClaimVariant {
  source: string;
  link: string;
  claim: string;
  confidence: number;
...
export interface AgentContradictionFlag {
  id: string;
  sourceA: string;
  sourceB: string;
  claimA: string;
...
export interface AgentResearchPack {
  canonicalClaim: string;
  claimVariants: AgentClaimVariant[];
  evidence: AgentEvidenceItem[];
  contradictionFlags: AgentContradictionFlag[];
  sourceCount: number;
}

export interface EmergingEventCluster {
  clusterId: string;
...
interface StoryResearchInput {
  primary: FeedItem;
  related: FeedItem[];
  fallbackClaim: string;
  primarySummary?: string;
  relatedSummaryByLink?: Record<string, string>;
}
...
export function buildAgentResearchPack(input: StoryResearchInput): AgentResearchPack {
  ...
  const summaryByLink: Record<string, string> = {
...
export function runResearchSwarm(items: FeedItem[], maxClusters = 20): AgentSwarmOutput {
...
function clusterEmergingEvents(items: FeedItem[]): EmergingEventCluster[] {
  ...
  const clusters: EmergingEventCluster[] = [];
  for (const indices of groups.values()) {
    ...
    clusters.push({
      ...
      entityHash: computeEntityHash(identifier),
      title: primary.title,
      canonicalClaim,
...
function isLikelySameEvent(a: ItemSignals, b: ItemSignals): boolean {
  const timeDiffHours = Math.abs(a.pubMs - b.pubMs) / (1000 * 60 * 60);
  if (timeDiffHours > 96) return false;
  ...
  if (titleOverlap >= 2 && tokenJaccard >= 0.14) return true;
  if (tagOverlap >= 2 && tokenJaccard >= 0.12) return true;
  return tokenJaccard >= 0.2 && titleOverlap >= 1;
}

function buildEvidence(items: FeedItem[], summaryByLink: Record<string, string>): AgentEvidenceItem[] {
...
function inferEvidenceKind(item: FeedItem): AgentEvidenceKind {
...
function extractClaimVariant(item: FeedItem): AgentClaimVariant {
...
function chooseCanonicalClaim(variants: AgentClaimVariant[], fallback: string): string {
  if (variants.length === 0) return normalizeClaim(fallback);
...
function sourceFactualityWeight(source: string): number {
  const s = source.toLowerCase();
  if (s.includes("reuters") || s.includes("associated press") || s.includes("afp")) return 0.08;
  if (s.includes("bbc") || s.includes("npr") || s.includes("financial times") || s.includes("wall street journal")) return 0.05;
  return 0;
}

function estimateClaimConfidence(claim: string, item: FeedItem): number {
  ...
  if (hasAction) score += 0.17;
  if (hasProperName) score += 0.11;
  if (hasNumber) score += 0.05;
  if (hasQuestionForm) score -= 0.2;
  if (hasQuote) score -= 0.04;
...
function dedupeContradictions(flags: AgentContradictionFlag[]): AgentContradictionFlag[] {
  const seen = new Set<string>();
  const output: AgentContradictionFlag[] = [];
...
function toSignals(item: FeedItem): ItemSignals {
  return {
...
function mergeTags(items: FeedItem[]): string[] {
  const out = new Set<string>();
  for (const item of items) {
    out.add(item.category.toLowerCase());
    for (const tag of item.tags || []) out.add(tag.toLowerCase());
  }
