...
import { remember, recall, countByScope } from "./memory";
...
async function extractFactsWithDebug(text: string): Promise<{ facts: string[]; debug: string }> {
  let result;
  try {
    result = await generateTextForTask({
      task: "factExtraction",
...
export async function learnFromUrl(url: string): Promise<LearnResult> {
  try {
    ...
    for (let i = 0; i < facts.length; i++) {
      await remember("knowledge", `${slug}-${i}`, facts[i]);
    }

    // Store source metadata
    await remember("knowledge-sources", slug, JSON.stringify({
      url,
...
export async function getKnowledgeStats(): Promise<KnowledgeStats> {
  ...
  const sourceEntries = await recall("knowledge-sources");
 ...
 */
export async function buildKnowledgeContext(maxFacts = 40): Promise<string | null> {
