...
// Illustration generation moved to /api/cron/daily-illustration (separate cron)
// to avoid exceeding Vercel's 55s maxDuration.
...
async function generateDailyEdition(data: DailyEditionData): Promise<DailyEditionResult | null> {
  ...
  // Save editorial IMMEDIATELY — don't wait for illustration
  try {
  ...
  // NOTE: Illustration (DALL-E 3 cover art) is generated by a SEPARATE cron
  // endpoint (/api/cron/daily-illustration) that runs 15 min after this one.
  // This avoids exceeding Vercel's 55s maxDuration — writer+extractor already
  // consume most of the budget. The illustration cron reads this editorial,
  // generates the art, and updates hasIllustration=true.
  console.log("[daily-edition] Editorial saved. Illustration will be generated by separate cron.");
