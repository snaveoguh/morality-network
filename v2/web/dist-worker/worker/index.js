...
const agent_swarm_1 = require("../lib/agent-swarm");
const bridge_signature_1 = require("../lib/agents/core/bridge-signature");
const runtime_mode_1 = require("../lib/runtime-mode");
const engine_1 = require("../lib/trading/engine");
const config_1 = require("../lib/trading/config");
const scalper_1 = require("../lib/trading/scalper");
const DEFAULT_TASKS = ["scanner", "swarm"];
const VALID_TASKS = new Set(["scanner", "swarm", "trader", "bridge"]);
...
function parseIntegerEnv(key, fallback, minValue = 1) {
...
function getIndexerBaseUrl() {
    const raw = process.env.INDEXER_BACKEND_URL ||
...
function getEnabledTasks() {
    ...
    const parsed = raw
        ...
        .filter((task) => VALID_TASKS.has(task));
    return parsed.length > 0 ? parsed : DEFAULT_TASKS;
}
async function postIndexer(path, body) {
...
async function fetchIndexerJson(path) {
    const baseUrl = getIndexerBaseUrl();
    const response = await fetch(new URL(path, `${baseUrl}/`).toString(), {
        method: "GET",
        headers: buildAuthHeaders(),
...
async function fetchBridgeCursor(consumer) {
    const payload = await fetchIndexerJson(`/api/v1/agents/cursors/${encodeURIComponent(consumer)}`);
    return {
        lastEventId: payload.cursor?.lastEventId ?? null,
        lastTimestampMs: Number(payload.cursor?.lastTimestampMs ?? 0),
    };
}
async function updateBridgeCursor(consumer, cursor) {
...
function trimBridgeBacklog(events, cursor) {
    if (cursor.lastTimestampMs <= 0)
        return events;
...
async function relayBridgeEvent(event) {
...
async function runSwarmTask() {
    const items = await (0, rss_1.fetchAllFeeds)(rss_1.DEFAULT_FEEDS);
    const output = (0, agent_swarm_1.runResearchSwarm)(items, parseIntegerEnv("WORKER_SWARM_CLUSTERS", 30, 1));
...
async function runTraderTask() {
    if ((0, runtime_mode_1.getTraderExecutionMode)() !== "worker") {
        log("trader task skipped because TRADER_EXECUTION_MODE is not worker");
        return;
    }
    const cycles = await (0, engine_1.runTraderCycles)();
    const [readinessByRunner, performanceByRunner, positionsByRunner] = await Promise.all([
        (0, engine_1.getTraderReadinessByRunner)(),
        (0, engine_1.getTraderPerformanceByRunner)(),
        (0, engine_1.listTraderPositionsByRunner)(),
    ]);
    await postIndexer("/api/v1/trading/state", {
        executionMode: "worker",
        config: (0, engine_1.redactedConfigSummary)(),
...
async function runBridgeTask() {
    ...
    const payload = await fetchIndexerJson(`/api/v1/agents/events?${query.toString()}`);
...
async function runStartup(tasks) {
    let success = true;
...
function scheduleTask(task, intervalMs) {
    return (0, node_timers_1.setInterval)(() => {
...
async function main() {
    ...
    log("starting", {
        ...
        traderMode: (0, runtime_mode_1.getTraderExecutionMode)(),
    });
    if (runOnStart || once) {
    ...
    let scalper = null;
    const scalperConfig = (0, config_1.getScalperConfig)();
    if (scalperConfig.enabled && (0, runtime_mode_1.getTraderExecutionMode)() === "worker") {
        try {
            const traderConfig = (0, config_1.getTraderConfig)();
            if (traderConfig.executionVenue === "hyperliquid-perp") {
                scalper = new scalper_1.ScalperManager(traderConfig, scalperConfig);
                await scalper.start();
