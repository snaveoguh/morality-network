...
exports.runSelfLearn = runSelfLearn;
...
async function indexerFetch(path, timeoutMs = 15_000) {
...
async function storeFacts(pipeline, rangeLabel, facts) {
    let stored = 0;
    for (let i = 0; i < facts.length; i++) {
        try {
            await (0, memory_1.remember)("knowledge", `${pipeline}-${rangeLabel}-${i}`, facts[i]);
            stored++;
        }
        catch (err) {
            console.warn(`[self-learn:${pipeline}] failed to store fact ${i}:`, err instanceof Error ? err.message : err);
        }
    }
    return stored;
}
async function getProgress(pipeline) {
    const entries = await (0, memory_1.recall)("self-learn-progress", `last-${pipeline}`);
    return entries.length > 0 ? entries[0].content : null;
}
async function setProgress(pipeline, checkpoint) {
    await (0, memory_1.remember)("self-learn-progress", `last-${pipeline}`, checkpoint);
}
...
async function learnFromSentimentTrends() {
    const result = { pipeline: "sentiment-trends", factsStored: 0, errors: [] };
    try {
        // Fetch AI usage summary for the last 24 hours
        const summary = await indexerFetch("/api/v1/ai/usage/summary?hours=24");
        if (!summary) {
            result.errors.push("No AI usage summary available");
            return result;
        }
        const aiResult = await (0, ai_provider_1.generateTextForTask)({
            task: "selfLearn",
...
Output a JSON array of 10-15 concise fact strings. Each fact should be a single, self-contained observation.`;
async function learnFromEventCorpus() {
    const result = { pipeline: "event-corpus", factsStored: 0, errors: [] };
    try {
        ...
        const eventsData = await indexerFetch(`/api/v1/agents/events?${params.toString()}`);
...
Output a JSON array of 10-15 concise fact strings. Each fact should be a single, self-contained observation.`;
async function learnFromSourceQuality() {
    const result = { pipeline: "source-quality", factsStored: 0, errors: [] };
    try {
        // Fetch recent articles from the archive
        const archiveData = await indexerFetch("/api/v1/archive/articles?limit=100&sort=desc");
...
Output a JSON array of 10-15 concise fact strings. Each fact should be a single, self-contained observation.`;
async function learnFromEditorialArchive() {
    const result = { pipeline: "editorial-archive", factsStored: 0, errors: [] };
    try {
        const editorialData = await indexerFetch("/api/v1/archive/editorials?limit=50&sort=desc");
...
function parseFactsFromResponse(text) {
 ...
 */
async function runSelfLearn() {
    console.log("[self-learn] starting all pipelines...");
    const [sentimentTrends, eventCorpus, sourceQuality, editorialArchive] = await Promise.all([
    ...
    const totalFacts = sentimentTrends.factsStored +
