...
exports.getMoralCompassSynthesis = getMoralCompassSynthesis;
exports.buildMoralCompassContext = buildMoralCompassContext;
exports.getMoralCompassStats = getMoralCompassStats;
exports.runMoralCompassPipeline = runMoralCompassPipeline;
require("server-only");
...
function principleId(category, tradition, statement) {
...
async function getCrawledSources() {
    const entries = await (0, memory_1.recall)(SCOPE_META, "crawled-sources");
    if (entries.length === 0)
        return new Set();
    try {
        const urls = JSON.parse(entries[0].content);
        return new Set(urls);
    }
    catch {
        return new Set();
...
async function saveCrawledSources(crawled) {
    await (0, memory_1.remember)(SCOPE_META, "crawled-sources", JSON.stringify([...crawled]));
}
async function getDiscoveredSources() {
    const entries = await (0, memory_1.recall)(SCOPE_META, "discovered-sources");
    if (entries.length === 0)
        return [];
    try {
...
async function saveDiscoveredSources(sources) {
    ...
    await (0, memory_1.remember)(SCOPE_META, "discovered-sources", JSON.stringify(sources.slice(0, 50)));
}
...
// ─── Principle storage ──────────────────────────────────────────────────────
async function loadAllPrinciples() {
    const entries = await (0, memory_1.recall)(SCOPE_PRINCIPLES);
    const principles = [];
...
async function storePrinciple(principle) {
    await (0, memory_1.remember)(SCOPE_PRINCIPLES, principle.id, JSON.stringify(principle));
}
// ─── Core pipeline functions ────────────────────────────────────────────────
function validateCategory(cat) {
    return VALID_CATEGORIES.has(cat) ? cat : null;
}
function validateAxes(axes) {
    if (!Array.isArray(axes))
        return ["harm"];
    return axes.filter((a) => typeof a === "string" && VALID_AXES.has(a));
}
async function extractPrinciplesFromUrl(source) {
    try {
...
async function reinforceOrStore(incoming, existing) {
    ...
    for (const ex of existing) {
        if (ex.category !== incoming.category)
            continue;
...
async function discoverNewSources(recentUrls) {
    ...
    try {
        ...
        for (const raw of parsed.slice(0, 5)) {
            ...
            if (!url || !url.startsWith("https://") || allKnownUrls.has(url))
                continue;
            const category = validateCategory(typeof raw.category === "string" ? raw.category : "");
            if (!category)
                continue;
        ...
        console.warn("[moral-compass] source discovery failed:", err instanceof Error ? err.message : err);
        return [];
    }
}
async function decayAndPrune() {
    ...
    for (const p of principles) {
        ...
        if (age > decayThreshold) {
            ...
            if (p.weight < PRUNE_THRESHOLD) {
                await (0, memory_1.forget)(SCOPE_PRINCIPLES, p.id);
                pruned++;
            }
            else {
...
async function synthesize(principles) {
    ...
    const principleText = ranked
        .map((p, i) => `${i + 1}. [${p.category}/${p.tradition}] (confidence: ${p.confidence.toFixed(2)}, sources: ${p.sourceCount})\n   ${p.statement}\n   Axes: ${p.relevantAxes.join(", ")}\n   Guidance: ${p.applicationGuidance}`)
        .join("\n\n");
    try {
        const result = await (0, ai_provider_1.generateTextForTask)({
            task: "moralCompass",
        ...
        await (0, memory_1.remember)(SCOPE_META, "synthesis", synthesis);
        await (0, memory_1.remember)(SCOPE_META, "last-synthesis", new Date().toISOString());
        console.log(`[moral-compass] synthesis updated: ${synthesis.length} chars from ${ranked.length} principles`);
        return true;
    }
    catch (err) {
        console.error("[moral-compass] synthesis failed:", err instanceof Error ? err.message : err);
        return false;
    }
}
// ─── Public API ─────────────────────────────────────────────────────────────
 ...
 */
async function getMoralCompassSynthesis() {
    try {
        const entries = await (0, memory_1.recall)(SCOPE_META, "synthesis");
 ...
 */
async function buildMoralCompassContext() {
    try {
 ...
 */
async function getMoralCompassStats() {
    const principles = await loadAllPrinciples();
    const crawled = await getCrawledSources();
    ...
    const lastSynthesisEntries = await (0, memory_1.recall)(SCOPE_META, "last-synthesis");
 ...
 */
async function runMoralCompassPipeline() {
    ...
    console.log(`[moral-compass] pipeline complete: ${result.sourcesCrawled} crawled, ` +
