...
exports.DEFAULT_FEEDS = exports.KEYWORD_PATTERNS = void 0;
...
async function fetchFeed(source) {
    try {
        ...
        const bias = (0, bias_1.getSourceBias)(source.url) || (0, bias_1.getSourceBias)(source.name);
        return (feed.items || []).slice(0, 15).map((item) => {
            ...
            const feedItem = {
                ...
                canonicalClaim: (0, claim_extract_1.extractCanonicalClaim)({
                    title: item.title || "",
                    description: stripHtml(item.contentSnippet || item.content || ""),
                    url: item.link || "",
                }),
        ...
        console.error(`[RSS:${source.name}] Failed to fetch feed:`, error instanceof Error ? error.message : error);
        return [];
    }
}
// ============================================================================
...
async function fetchAllFeeds(sources = exports.DEFAULT_FEEDS) {
    ...
    const [rssResults, redditItems, chanItems] = await Promise.all([
        ...
        (0, scrapers_1.fetchRedditFeeds)().catch((err) => {
            console.error("[Scrapers] Reddit fetch failed:", err);
            return [];
        }),
        (0, scrapers_1.fetchChanFeeds)().catch((err) => {
    ...
    for (const item of [...redditItems, ...chanItems]) {
        ...
        if (!item.canonicalClaim) {
            item.canonicalClaim = (0, claim_extract_1.extractCanonicalClaim)({
                title: item.title,
...
// ============================================================================
function autoAssignTags(item) {
    const text = `${item.title} ${item.description}`.toLowerCase();
    const tags = [];
    for (const [tag, pattern] of Object.entries(exports.KEYWORD_PATTERNS)) {
...
async function fetchXmlFromUrl(url, sourceName, maxRetries = 3) {
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        ...
        try {
            ...
            console.warn(`[RSS:${sourceName}] ${res.status}, retrying in ${delay}ms (${attempt + 1}/${maxRetries})`);
            ...
            const delay = RETRY_BACKOFF_MS[attempt] ?? 2000;
            console.warn(`[RSS:${sourceName}] fetch error, retrying in ${delay}ms (${attempt + 1}/${maxRetries})`, error instanceof Error ? error.message : error);
            await new Promise((resolve) => setTimeout(resolve, delay));
        }
        finally {
...
async function fetchFeedXmlWithRetry(source, maxRetries = 3) {
    try {
        ...
        if (error instanceof FallbackError && source.fallbackUrl) {
            console.warn(`[RSS:${source.name}] Primary URL returned ${error.statusCode}, trying fallback: ${source.fallbackUrl}`);
            try {
                const xml = await fetchXmlFromUrl(source.fallbackUrl, `${source.name}:fallback`, maxRetries);
                console.log(`[RSS:${source.name}] Fallback URL succeeded: ${source.fallbackUrl}`);
                return xml;
            }
            catch (fallbackError) {
                console.error(`[RSS:${source.name}] Fallback URL also failed:`, fallbackError instanceof Error ? fallbackError.message : fallbackError);
