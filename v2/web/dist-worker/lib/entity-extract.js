...
function classifyEntity(name) {
    ...
    if (orgWords.some(w => name.includes(w)))
        return "organization";
...
function isValidEntity(name) {
    ...
    if (STOP_WORDS.has(name))
        return false;
    if (STOP_WORDS.has(name.split(" ")[0]))
        return false;
...
function extractEntities(paragraphs) {
    ...
    for (let pIdx = 0; pIdx < paragraphs.length; pIdx++) {
        ...
        while ((match = SINGLE_NAME_RE.exec(paragraph)) !== null) {
            ...
            if (!COUNTRIES.has(name) && !ORGANIZATIONS.has(name))
                continue;
            const canonicalName = name.toLowerCase();
            const occurrence = {
                paragraphIndex: pIdx,
 ...
 */
function enrichEntities(entities, paragraphs, biasContext) {
    return entities.map(entity => {
        ...
        const sentence = paragraph.slice(sentenceStart > -1 ? sentenceStart + 1 : 0, sentenceEnd > -1 ? sentenceEnd + 1 : paragraph.length).trim();
