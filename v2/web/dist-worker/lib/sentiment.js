...
exports.TOPIC_TAXONOMY = void 0;
exports.matchesTopicDefinition = matchesTopicDefinition;
exports.computeSentimentSnapshot = computeSentimentSnapshot;
exports.fetchMarketData = fetchMarketData;
exports.sentimentLabel = sentimentLabel;
exports.trendArrow = trendArrow;
exports.trendArrowPercent = trendArrowPercent;
exports.formatPercentTrend = formatPercentTrend;
...
exports.TOPIC_TAXONOMY = [
    ...
    { slug: "ai", displayName: "Artificial Intelligence", category: "thematic", pattern: rss_1.KEYWORD_PATTERNS.ai || /\b(ai|artificial\s*intelligence|machine\s*learning|llm|openai|deepmind|chatgpt)\b/i, marketId: null, symbol: "🧠" },
    ...
    { slug: "trade", displayName: "Trade & Tariffs", category: "thematic", pattern: rss_1.KEYWORD_PATTERNS.trade || /\b(tariff|trade\s*war|sanction|export|import|trade\s*deal|embargo|protectionism)\b/i, marketId: null, symbol: "📦" },
    { slug: "war", displayName: "Conflict & Security", category: "thematic", pattern: rss_1.KEYWORD_PATTERNS.war || /\b(war|invasion|airstrike|missile|troops|military|ceasefire|bombing)\b/i, marketId: null, symbol: "⚔" },
    { slug: "election", displayName: "Elections & Democracy", category: "thematic", pattern: rss_1.KEYWORD_PATTERNS.election || /\b(election|ballot|poll|voter|campaign|candidate|primary|referendum)\b/i, marketId: null, symbol: "🗳" },
    { slug: "crypto", displayName: "Crypto & Web3", category: "thematic", pattern: rss_1.KEYWORD_PATTERNS.crypto || /\b(crypto|blockchain|defi|nft|web3|token|stablecoin|dao)\b/i, marketId: null, symbol: "⛓" },
    { slug: "economy", displayName: "Global Economy", category: "thematic", pattern: rss_1.KEYWORD_PATTERNS.economy || /\b(economy|gdp|recession|growth|unemployment|market|inflation)\b/i, marketId: null, symbol: "📈" },
    { slug: "rights", displayName: "Human Rights", category: "thematic", pattern: rss_1.KEYWORD_PATTERNS.rights || /\b(rights|freedom|protest|justice|equality|discrimination|civil\s*liberty)\b/i, marketId: null, symbol: "⚖" },
    { slug: "health", displayName: "Health & Pandemic", category: "thematic", pattern: rss_1.KEYWORD_PATTERNS.health || /\b(health|vaccine|pandemic|disease|medical|hospital|outbreak|who)\b/i, marketId: null, symbol: "🏥" },
    { slug: "scandal", displayName: "Scandal & Corruption", category: "thematic", pattern: rss_1.KEYWORD_PATTERNS.scandal || /\b(scandal|corruption|fraud|indictment|bribery|embezzlement|coverup)\b/i, marketId: null, symbol: "🔍" },
];
// ── Sentiment Lexicon ───────────────────────────────────────────────────────
 ...
 */
function computeSentimentSignal(items) {
    if (items.length === 0)
        return 50;
    let weightedSum = 0;
    let totalWeight = 0;
 ...
 */
function computeVolumeSignal(items) {
    const now = Date.now();
 ...
 */
function computeContradictionSignal(items) {
    if (items.length < 2)
        return { score: 70, ratio: 0 };
    // Build claim variants from articles
    const variants = items.slice(0, 50).map((item) => ({
        source: item.source,
        link: item.link,
        claim: item.canonicalClaim || item.title,
        confidence: 0.5,
    }));
    const contradictions = (0, agent_swarm_1.detectContradictions)(variants);
 ...
 */
function computeMarketSignal(changePct) {
 ...
 */
function computeBiasSpreadSignal(items) {
    const biases = [];
    ...
    for (const item of items) {
        for (const sourceName of itemSourceNames(item)) {
            ...
            const bias = (0, bias_1.getSourceBias)(sourceName);
            if (bias)
                biases.push(bias);
        }
    }
    if (biases.length < 2)
        return 50;
    const positions = biases.map((b) => (0, bias_1.biasToPosition)(b.bias));
 ...
 */
function estimateKeywordSeverity(items) {
    if (items.length === 0)
        return 20;
    let highHits = 0;
    let medHits = 0;
    ...
    for (const item of items.slice(0, 50)) {
        ...
        for (const word of words) {
            if (HIGH_SEVERITY_TERMS.has(word))
                highHits++;
            if (MED_SEVERITY_TERMS.has(word))
                medHits++;
        }
    }
    if (totalWords === 0)
...
function computeCompositeScore(signals) {
    ...
    const raw = signals.sentimentScore * weights.sentiment +
...
function matchesTopicDefinition(item, topic) {
    const text = `${item.title} ${item.description || ""}`;
    return topic.pattern.test(text);
}
function computeTopicSentiment(topic, allItems, marketData, previousScore, aiScores) {
    ...
    const rawArticleCount = items.reduce((sum, item) => sum + (item.rawArticleCount ?? 1), 0);
 ...
 */
function computeSentimentSnapshot(allItems, marketData, previousSnapshot, aiScores) {
    ...
    const topics = exports.TOPIC_TAXONOMY.map((topic) => computeTopicSentiment(topic, allItems, marketData, previousScores.get(topic.slug) ?? null, aiScores ?? null));
...
// ── Market Data Fetcher ─────────────────────────────────────────────────────
const COINGECKO_IDS = exports.TOPIC_TAXONOMY
    .filter((t) => t.marketId)
    .map((t) => t.marketId)
    .join(",");
/**
 ...
 */
async function fetchMarketDataUncached() {
    if (!COINGECKO_IDS)
        return { priceChanges: {} };
    try {
...
async function fetchMarketData() {
    return (0, ttl_cache_1.loadTtlValue)(marketDataCache, "market-data", MARKET_DATA_CACHE_TTL_MS, fetchMarketDataUncached);
}
// ── Sentiment Label ─────────────────────────────────────────────────────────
...
function trendArrowPercent(pctChange) {
    if (pctChange === null)
        return "\u2014";
    if (pctChange > 1)
        return "\u25B2";
...
function formatPercentTrend(pctChange) {
    if (pctChange === null)
        return "\u2014";
