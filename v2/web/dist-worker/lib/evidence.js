...
exports.parseEvidenceInput = parseEvidenceInput;
...
function includesAny(host, hints) {
    const lc = host.toLowerCase();
    return hints.some((hint) => lc.includes(hint));
}
function classifySourceType(host) {
    if (includesAny(host, GOV_HOST_HINTS))
        return "government-record";
    if (includesAny(host, DAO_HOST_HINTS))
        return "dao-governance";
    if (includesAny(host, RESEARCH_HOST_HINTS))
        return "research";
    if (includesAny(host, SOCIAL_HOST_HINTS))
        return "social-post";
    if (includesAny(host, NEWS_HOST_HINTS))
        return "news-report";
...
function parseEvidenceInput(input) {
    ...
    if (!raw) {
        return {
            ...
            isValidUrl: false,
            normalized: "",
            evidenceHash: (0, viem_1.keccak256)((0, viem_1.toBytes)("")),
            host: "",
            sourceType: "unknown",
            qualityTier: "low",
    ...
    try {
        ...
        return {
            ...
            isValidUrl: true,
            normalized,
            evidenceHash: (0, viem_1.keccak256)((0, viem_1.toBytes)(normalized)),
            host,
            sourceType,
            qualityTier,
