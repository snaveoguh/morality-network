...
exports.extractCanonicalClaim = extractCanonicalClaim;
...
const WEAK_CLAIM_PATTERN = /^(live update|latest update|opinion|analysis|watch|listen|read|newsletter|podcast)\b/i;
...
function removeNoisePrefix(text) {
    let value = text;
...
function isWeakClaim(text) {
    const trimmed = text.trim();
    if (!trimmed)
        return true;
    if (trimmed.length < 18)
        return true;
    if (WEAK_CLAIM_PATTERN.test(trimmed))
        return true;
    return false;
}
function claimFromUrl(url) {
    if (!url)
        return "";
    try {
