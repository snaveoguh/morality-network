...
exports.positionToJournalEntry = positionToJournalEntry;
exports.positionsToJournal = positionsToJournal;
exports.computePerformanceMetrics = computePerformanceMetrics;
/* ═══════════════════  Position → Journal  ═══════════════════ */
function positionToJournalEntry(p) {
    if (p.status !== "closed" || p.exitPriceUsd === undefined)
        return null;
    ...
    return {
        ...
        pnlUsd,
        pnlPct,
        holdDurationMs: p.closedAt && p.openedAt ? p.closedAt - p.openedAt : undefined,
...
function positionsToJournal(positions) {
    return positions
        .map(positionToJournalEntry)
        .filter((e) => e !== null)
        .sort((a, b) => (a.exitTimestamp ?? 0) - (b.exitTimestamp ?? 0));
}
function computePerformanceMetrics(journal) {
    ...
    const grossLossUsd = Math.abs(losses.reduce((s, t) => s + (t.pnlUsd ?? 0), 0));
    ...
    const avgHoldDurationMs = holdDurations.length > 0
        ...
        : 0;
    const avgLeverage = closed.reduce((s, t) => s + t.leverage, 0) / closed.length;
    // By symbol
    const bySymbol = {};
    for (const t of closed) {
        const sym = t.symbol;
