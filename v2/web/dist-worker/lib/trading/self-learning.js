...
exports.computeMarketKelly = computeMarketKelly;
exports.scoreSignalSources = scoreSignalSources;
exports.computeAdaptiveWeights = computeAdaptiveWeights;
exports.analyzeStreaks = analyzeStreaks;
exports.generateLearningReport = generateLearningReport;
const trade_journal_1 = require("./trade-journal");
function computeMarketKelly(journal) {
...
function scoreSignalSources(journal) {
 ...
 */
function computeAdaptiveWeights(journal) {
    const defaults = {
        technical: 0.4,
        pattern: 0.3,
        news: 0.3,
    ...
    const hasEnoughData = [techScore, patternScore, newsScore].some((s) => s && s.trades >= 5);
    if (!hasEnoughData)
        return defaults;
...
function analyzeStreaks(journal) {
    if (journal.length === 0) {
        return {
            currentStreak: 0,
            maxWinStreak: 0,
    ...
    // Sort by exit time
    const sorted = [...journal].sort((a, b) => (a.exitTimestamp ?? 0) - (b.exitTimestamp ?? 0));
    let currentStreak = 0;
    let maxWinStreak = 0;
    let maxLossStreak = 0;
    let winStreak = 0;
...
function generateLearningReport(positions) {
    const journal = positions
        .filter((p) => p.status === "closed")
        .map(trade_journal_1.positionToJournalEntry)
        .filter((e) => e !== null);
