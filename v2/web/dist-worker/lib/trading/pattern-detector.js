...
function buildIndicatorSummary(tech) {
...
/* ═══════════════════  Main export  ═══════════════════ */
async function detectPatterns(config, symbol, technicalSignal, candles) {
    const candleData = candles ?? (await (0, hyperliquid_1.fetchCandles)(config, symbol, "15m", 50));
    const userPrompt = [
