...
exports.computeKelly = computeKelly;
exports.consecutiveLosses = consecutiveLosses;
/* ═══════════════════  Core Kelly Computation  ═══════════════════ */
/**
 * Compute raw Kelly fraction: f* = (bp - q) / b
 */
function rawKellyFraction(winProb, winLossRatio) {
 ...
 */
function statsFromJournal(trades, symbol) {
    const closed = trades.filter((t) => t.pnlUsd !== undefined && (!symbol || t.symbol === symbol));
    if (closed.length === 0) {
 ...
 */
function kellyPhase(tradeCount) {
    if (tradeCount >= 100)
        return { phase: "hot", multiplier: 2 / 3 };
    if (tradeCount >= 10)
        return { phase: "warm", multiplier: 0.5 };
    return { phase: "cold", multiplier: 0.25 };
}
/**
 * Check recent drawdown. If > threshold, force quarter-Kelly.
 */
function recentDrawdownPct(trades, windowSize = 20) {
    const recent = trades
...
function computeKelly(args) {
    ...
    const leverage = Math.min(Math.max(1, Math.floor(positionNotionalUsd / accountValueUsd)), maxLeverage);
 ...
 */
function consecutiveLosses(journal) {
    const closed = journal.filter((t) => t.pnlUsd !== undefined);
    let streak = 0;
    for (let i = closed.length - 1; i >= 0; i--) {
        if ((closed[i].pnlUsd ?? 0) < 0)
            streak++;
        else
            break;
    }
