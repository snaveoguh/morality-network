...
exports.getLatestAdvisory = getLatestAdvisory;
exports.parseVeniceRiskResponse = parseVeniceRiskResponse;
exports.getAdvisoryForSymbol = getAdvisoryForSymbol;
exports.pollVeniceRiskAdvisory = pollVeniceRiskAdvisory;
...
async function redisGetAdvisory() {
    if (!redisEnabled())
        return null;
    try {
...
async function redisSetAdvisory(advisory) {
    if (!redisEnabled())
        return false;
    try {
...
// ── Store ────────────────────────────────────────────────────────────────────
async function getLatestAdvisory() {
    ...
    // Filesystem fallback
    try {
        const raw = await node_fs_1.promises.readFile(FS_PATH, "utf-8");
...
async function storeAdvisory(advisory) {
    const redisOk = await redisSetAdvisory(advisory);
    if (!redisOk) {
        try {
            await node_fs_1.promises.writeFile(FS_PATH, JSON.stringify(advisory), "utf-8");
        }
        catch { /* not fatal */ }
    }
}
// ── Structured Venice Prompt ─────────────────────────────────────────────────
function buildStructuredRiskPrompt(positions, deployedUsd, accountValueUsd, unrealizedPnlUsd, realizedPnlUsd) {
    const posLines = positions.length > 0
        ? positions.map(p => `  ${p.symbol}: dir=${p.direction ?? "long"} lev=${p.leverage ?? 1}x entry=$${p.entryPrice.toFixed(2)} current=${p.currentPrice !== null ? `$${p.currentPrice.toFixed(2)}` : "n/a"} size=$${p.size.toFixed(2)} pnl=${p.unrealizedPnl !== null ? `$${p.unrealizedPnl.toFixed(2)}` : "n/a"}`).join("\n")
        : "  (no open positions)";
    const concLines = positions.length > 0 && deployedUsd > 0
        ? positions.map(p => `  ${p.symbol}: ${(p.size / deployedUsd * 100).toFixed(1)}%`).join("\n")
        : "  n/a";
...
function parseVeniceRiskResponse(rawText) {
        ...
        try {
            ...
            if (Array.isArray(parsed.directives)) {
                const directives = parsed.directives
                    ...
                    action: VALID_ACTIONS.has(d.action) ? d.action : "allow",
                    reason: String(d.reason).slice(0, 200),
                    severity: VALID_SEVERITIES.has(d.severity) ? d.severity : "info",
 ...
 */
function getAdvisoryForSymbol(advisory, symbol) {
    if (!advisory || advisory.directives.length === 0)
        return null;
    const upper = symbol.toUpperCase();
    const matching = advisory.directives.filter(d => d.symbol === upper || d.symbol === "PORTFOLIO");
...
// ── Venice Risk Poller ───────────────────────────────────────────────────────
async function pollVeniceRiskAdvisory() {
    ...
    // Build context from engine performance data
    // @ts-ignore Node16 moduleResolution requires .js but formatters remove it
    const { getTraderPerformance } = await import("./engine");
    const perf = await getTraderPerformance();
    const positions = perf.open.map((o) => ({
        ...
        leverage: o.position.leverage,
    }));
    const prompt = buildStructuredRiskPrompt(positions, perf.totals.deployedUsd, perf.totals.openMarketValueUsd || perf.totals.deployedUsd, perf.totals.unrealizedPnlUsd, perf.totals.realizedPnlUsd);
    ...
    try {
        ...
        console.log(`[risk-advisory] polled Venice: ${directives.length} directives (parse=${parseStatus})`);
        return advisory;
    }
    finally {
