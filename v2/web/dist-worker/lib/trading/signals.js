...
exports.getAggregatedMarketSignals = getAggregatedMarketSignals;
require("server-only");
const editorial_archive_1 = require("../editorial-archive");
const event_corpus_1 = require("../event-corpus");
...
const SYMBOL_PATTERNS = [
    ...
    {
        symbol: "DXY",
        pattern: /\b(dxy|dollar|usd|us treasury|treasury yields?|rates?|inflation|fomc|macro|trade flows?|political risk)\b/i,
    },
    {
        symbol: "SPX",
        pattern: /\b(spx|s&p\s*500|nasdaq(?:-100)?|qqq|equit(?:y|ies)|stocks?|etf|consumer staples?|pharma(?:ceutical)?|biotech|healthcare|semiconductor|haleon|corporate)\b/i,
    },
];
...
function extractTickerFromAsset(asset) {
...
function mapAssetToSymbol(asset, ticker) {
    ...
    if (embeddedTickerAlias)
        return embeddedTickerAlias;
...
function directionToSigned(direction) {
...
function computeRecencyWeight(generatedAt) {
...
function computeHorizonWeight(horizons) {
    if (!horizons.length)
        return 1;
...
const FALLBACK_TOPIC_TO_SYMBOL = {
    btc: "BTC",
    eth: "ETH",
    gold: "GOLD",
    oil: "OIL",
...
async function getFallbackFeedSignals() {
    try {
        // Only fetch market-relevant feeds — skip world news / science / reddit / 4chan
        const marketFeeds = rss_1.DEFAULT_FEEDS.filter((feed) => SIGNAL_FEED_CATEGORIES.has(feed.category));
        // If no market feeds matched (categories changed), fall back to full list but with tight timeout
        const feedsToFetch = marketFeeds.length > 0 ? marketFeeds : rss_1.DEFAULT_FEEDS;
        // Race the entire fallback against a 20s timeout (Vercel serverless limit is typically 30s)
        const items = await withTimeout((0, rss_1.fetchAllFeeds)(feedsToFetch), 20_000, []);
        if (items.length === 0)
            return [];
        const snapshot = await withTimeout((0, event_corpus_1.computeEventShapedSentimentSnapshotFromFeeds)(items, null), 15_000, null);
        if (!snapshot)
            return [];
        const signals = snapshot.topics
            ...
            const definition = sentiment_1.TOPIC_TAXONOMY.find((candidate) => candidate.slug === topic.slug);
            const supportingClaims = definition
                ? items
                    .filter((item) => (0, sentiment_1.matchesTopicDefinition)(item, definition))
            ...
            return {
                ...
                supportingClaims,
                contradictionPenalty: Math.max(0, Math.min(1, safeFinite(topic.signals.contradictionRatio, 0))),
...
// ── Main Aggregator ────────────────────────────────────────────────────────
async function getAggregatedMarketSignals(options) {
    ...
    const records = await withTimeout((0, editorial_archive_1.listRecentMarketImpactRecords)(limit), 10_000, []);
    if (records.length === 0) {
    ...
    for (const record of records) {
        ...
        for (const market of record.marketImpact.affectedMarkets) {
            ...
            const absDelta = safeFinite(confidence * significanceWeight * recencyWeight * horizonWeight, 0);
            if (absDelta <= 0)
                continue;
    ...
    console.log(`[signals] aggregated ${records.length} records, ${totalProcessed} markets (${totalMapped} mapped, ${totalUnmapped} unmapped), ${bySymbol.size} tickers`);
    const signals = Array.from(bySymbol.entries())
        ...
        const direction = dampedScore >= 0 ? "bullish" : "bearish";
        ...
        return {
            ...
            latestGeneratedAt: acc.latestTs > 0 ? new Date(acc.latestTs).toISOString() : new Date(0).toISOString(),
        ...
        .filter((s) => s !== null && s.score >= minAbsScore)
